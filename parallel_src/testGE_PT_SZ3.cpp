...
int main(int argc, char ** argv){
    ...
    for(int i=0; i<n_variable; i++){
		...
		counts.push_back(n);
		MPI_File file;
		std::string filename = "/pscratch/xli281_uksr/xliang/GE/PT_PSZ3_" + std::to_string(target_rel_eb) + "_" + var_name_out[i] + "_aggregated.dat";
	...
	// std::string offsets_file = "/project/xli281_uksr/xwu/test_GE/offsets/psz3_" + std::to_string(rank) + "_" + argv[1];
	// MGARD::writefile(offsets_file.c_str(), offsets.data(), offsets.size());
	// std::string count_file = "/project/xli281_uksr/xwu/test_GE/count/psz3_" + std::to_string(rank) + "_" + argv[1];
