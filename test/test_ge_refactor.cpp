...
const string dataset_prefix = "/Users/xliang/Dataset/GE/";
...
int main(int argc, char ** argv){
    ...
    int argv_id = 1;
    int zone_id = atoi(argv[argv_id ++]);
    string varname = string(argv[argv_id ++]);
    string filename = dataset_prefix + "data/zone_" + to_string(zone_id) + "_" + varname + ".dat";
    ...
    // string identifier = dataset_prefix + "refactor/zone_" + to_string(zone_id) + "_" + varname;
    string identifier = dataset_prefix + "refactor/" + varname;
    string metadata_file = identifier + "_refactored_data/metadata.bin";
    std::cout << "metadata: " << metadata_file << std::endl;
    vector<string> files;
    for(int i=0; i<=target_level; i++){
        string filename = identifier + "_refactored_data/level_" + to_string(i) + ".bin";
