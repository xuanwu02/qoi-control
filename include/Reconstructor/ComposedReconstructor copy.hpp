...
namespace MDR {
    ...
    private:
        bool reconstruct(uint8_t target_level, const std::vector<uint8_t>& prev_level_num_bitplanes, bool progressive=true){
            ...
            auto weights_interleaver = MDR::DirectInterleaver<int>();
            ...
            for(int i=0; i<=current_level; i++){
                if(level_num_bitplanes[i] - prev_level_num_bitplanes[i] > 0){
                    ...
                    if(negabinary) level_exp -= 2;
                    int * weights_buffer = (int *) malloc(level_elements[i] * sizeof(int));
                    weights_interleaver.interleave(weights, dimensions, level_dims[i], prev_dims, weights_buffer);
                    auto level_decoded_data = encoder.progressive_decode(level_components[i], level_elements[i], level_exp, prev_level_num_bitplanes[i], level_num_bitplanes[i] - prev_level_num_bitplanes[i], i, weights_buffer);
                    free(weights_buffer);
